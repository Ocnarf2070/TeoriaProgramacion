...
derivSeq st ini = [(Inter st ini)]++derivSeq' st ini
derivSeq' :: Stm -> State -> DerivSeq
derivSeq' st ini
	| isFinal conf = [conf]
		where
			conf = sosStm (Inter st ini)

derivSeq' st ini
	| otherwise = [conf] ++ derivSeq' st' s
		where
			conf = sosStm (Inter st ini)
			Inter st' s = conf
...
{- Formal definition of 'repeat S until b'
        ...
        [repeat-sos]  <Repeat S until b, s> => <S; if (Neg b) then ss else Skip, s>  - ss = Repeat S until b
...
factorial' = Comp (Ass "y" (N 1))
                 (Repeat (Comp (Ass "y" (Mult (V "y") (V "x")))
                          (Ass "x" (Sub (V "x") (N 1))))
													(Eq (V "x") (N 1)))
...
derivSeqAbort st ini = [(Inter st ini)]++derivSeqAbort' st ini
...
derivSeqAbort' st ini
	| otherwise = [conf] ++ derivSeqAbort' st' s
		where
			conf = sosStm (Inter st ini)
			Inter st' s = conf
...
aExpDerivSeq a ini = [Redex a ini] ++ aExpDerivSeq' a ini
aExpDerivSeq' :: Aexp -> State -> AexpDerivSeq
aExpDerivSeq' a ini
	| isValue red = [red]
		where
			red = sosAexp (Redex a ini)
aExpDerivSeq' a ini
	| otherwise = [red] ++ aExpDerivSeq' a' s
		where
			red = sosAexp (Redex a ini)
			Redex a' s = red
